...
ARTIFACT_DOWNLOAD_URL="${INSTALLER_DOWNLOAD_URL:-https://github.com/ferrocene/criticalup/releases/download/v%REPLACE_VERSION%}"
PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
if [ -n "${CRITICALUP_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$CRITICALUP_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${CRITICALUP_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${CRITICALUP_UNMANAGED_INSTALL:-}"
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.23.0"},"source":{"app_name":"criticalup","name":"criticalup","owner":"ferrocene","release_type":"github"},"version":"%REPLACE_VERSION%"}
EORECEIPT
RECEIPT_HOME="${HOME}/.config/criticalup"

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
criticalup-installer.sh

The installer for criticalup %REPLACE_VERSION%

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/ferrocene/criticalup/releases/download/v%REPLACE_VERSION%
...
USAGE:
    criticalup-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set CRITICALUP_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "criticalup-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _bins="criticalup"
            _bins_js_array='"criticalup"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "criticalup-aarch64-unknown-linux-gnu.tar.xz")
            ...
            _updater_bin=""
            ;;
        "criticalup-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _bins="criticalup.exe"
            _bins_js_array='"criticalup.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "criticalup-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="criticalup-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="criticalup-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="criticalup-aarch64-unknown-linux-gnu.tar.xz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="criticalup-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${CRITICALUP_INSTALL_DIR:-}" ]; then
        _force_install_dir="$CRITICALUP_INSTALL_DIR"
