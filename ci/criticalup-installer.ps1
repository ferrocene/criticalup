...
param (
    ...
    [string]$ArtifactDownloadUrl = 'https://github.com/ferrocene/criticalup/releases/download/v%REPLACE_VERSION%',
...
"@
$receipt_home = "${env:LOCALAPPDATA}\criticalup"

if ($env:CRITICALUP_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set CRITICALUP_NO_MODIFY_PATH=1 in the environment"
}

if ($env:CRITICALUP_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:CRITICALUP_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "criticalup-x86_64-pc-windows-msvc.zip"
      "bins" = @("criticalup.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\criticalup-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:CRITICALUP_INSTALL_DIR)) {
    $force_install_dir = $env:CRITICALUP_INSTALL_DIR
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/criticalup-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
