...
async fn product_dirs_are_created() {
    let test_env = TestEnvironment::prepare().await;
    let test_proj_repository = tempdir().unwrap();

    // Create a packet
    let package_ref = "my_test_packet";
    let product_ref = "my_test_product";
    let release_ref = "my_test_release";
    let package_dir = tempdir().unwrap();
    tokio::fs::write(package_dir.path().join("boop"), "swoop")
        .await
        .unwrap();

    let mut server = test_env.server();
    server
        .create_package(package_ref, product_ref, package_dir.path())
        .await
        .unwrap();
    server
        .create_release(release_ref, vec![package_ref])
        .unwrap();
