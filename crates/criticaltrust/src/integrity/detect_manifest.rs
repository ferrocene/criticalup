...
impl<'a> Iterator for ReverseSegmentsIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let result = loop {
            ...
            if let Some(pos) = self.cursor[..end].rfind(path::is_separator) {
...
mod tests {
    ...
    fn test_reverse_segments_iter_relative() {
        ...
        #[cfg(windows)] // Backslashes exist as legitimate path specifiers on Windows
        {
            assert_eq!(Some("r"), iter.next());
            assert_eq!(Some("ba"), iter.next());
        }
